...
if (directoryExists("platforms/android")) {
    ...
    if (fileExists( path )) {
        try {
            ...
            var oldFilePath = "platforms/android/res/values/strings.xml";
            var newFilePath = "platforms/android/app/src/main/res/values/strings.xml";
            var isOldDirectory = fileExists(oldFilePath);
            var isNewDirectory = fileExists(newFilePath);
            var path;
            if (isOldDirectory) {
                path = oldFilePath;
                ...
                throw new ReferenceError("Can't Find strings resource file");
            }

            var strings = fs.readFileSync(path).toString();
